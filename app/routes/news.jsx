...
import { deleteSearch } from "~/models/search.server";
...
export async function action({ request }) {
  if (request.method.toLowerCase() === "post") {
    ...
    if (_action === "delete") {
      ...
      // const latestId = allSearches[0].id;
      // await deleteSearch({ latestId, userId });
...
export async function loader({ request }) {
  ...
  try {
    ...
    if (allSearches.length >= 3) {
      const terms = allSearches.reduce((acc, search) => {
        acc.push(search.userSearchString, search.botSearchString);
        return acc;
      }, []);
      console.log(terms);
      const personalizedPage = await personalize(terms);
